...
const stripe = require('stripe')(process.env.STRIPE_SECRET_KEY || 'sk_test_51RF8S1PDMCfPLiP4EiQhKcxzF0CHI2eDuU4clkFCbb5l8PRZ2gJOAASfpInq6t9rlzoOyQYuEOGPuVLVFbbB801V009iKITh4a');
  ...
  async createStripePaymentIntent(email, amount) {
    try {
      ...
      const paymentIntent = await stripe.paymentIntents.create({
        amount: inrAmount, // Stripe requires amount in INR paise (smallest unit)
        currency: 'inr',
        metadata: { email, amount: amount.toString(), usdToInrRate: usdToInrRate.toString() },
        description: `Wallet top-up for ${email}`
      });
