...
// GET /api/transactions/history?email=user@example.com
// Returns all transactions (both debit and credit) for a user
router.get('/history', async (req, res) => {
  try {
    ...
    // Find all transactions for this user (both as sender and receiver)
    const transactions = await Transaction.findAll({
      where: {
    ...
    const enriched = await Promise.all(transactions.map(async (txn) => {
      ...
      return {
        ...
        description: txn.description,
        service: txn.metadata && txn.metadata.service ? txn.metadata.service : null,
        senderType: txn.senderType,
        receiverType: txn.receiverType
      };
    }));
